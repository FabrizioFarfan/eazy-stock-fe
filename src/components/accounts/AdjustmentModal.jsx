...
export default function AdjustmentModal({ entity, mutation, onClose, mode = 'customer' }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!notes.trim()) {
      setError(t('La nota es obligatoria para un ajuste — explicá el motivo'))
      return
    }
    if (direction === 'DECREASE' && amount > debt + 0.005) {
      setError(t('El ajuste DECREASE no puede superar la deuda actual de {amount}', { amount: formatPrice(debt) }))
      return
    }
    try {
      ...
      toast.success(t('Ajuste aplicado'))
      onClose()
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-5 py-4">
          ...
          <div className="flex flex-col gap-1">
            ...
            <textarea
              ...
              rows={3}
              placeholder={t('Explicá por qué se ajusta — queda en el historial.')}
          ...
          <div className="flex justify-end gap-2 border-t border-gray-200 -mx-5 px-5 pt-4">
            ...
            <button type="submit" disabled={mutation.isPending}
              ...
              {mutation.isPending ? t('Guardando...') : t('Aplicar ajuste')}
            </button>
          </div>
        </form>
      </div>
