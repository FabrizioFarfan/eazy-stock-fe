...
export default function PaymentModal({ entity, mutation, onClose, mode = 'customer' }) {
  ...
  const handleSubmit = async (e) => {
    ...
    if (amount > debt + 0.005) {
      setError(t('El pago no puede superar la deuda actual de {amount}', { amount: formatPrice(debt) }))
      return
    }
    try {
      ...
      toast.success(isSupplier
        ? t('Pago a {name} registrado', { name: entity.name })
        : t('Pago de {name} registrado', { name: entity.name }))
      onClose()
    } catch (err) {
      ...
      <div className="w-full max-w-md rounded-2xl bg-white shadow-xl">
        ...
        <div className="flex items-center justify-between border-b border-gray-200 px-5 py-4">
          <h3 className="text-base font-semibold text-gray-900">
            {isSupplier
              ? t('Registrar pago a {name}', { name: entity.name })
              : t('Registrar pago de {name}', { name: entity.name })}
          </h3>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 px-5 py-4">
          ...
          <div>
            <div className="mb-1.5 flex items-center justify-between gap-2">
              <span className="text-sm font-medium text-gray-700">{t('Monto del pago')}</span>
          ...
          {isSupplier && (
            <div className="flex flex-col gap-1">
              <label className="text-sm font-medium text-gray-700">{t('Referencia (opcional)')}</label>
              <input
                value={refDoc}
          ...
          <div className="flex justify-end gap-2 border-t border-gray-200 -mx-5 px-5 pt-4">
            ...
            <button type="submit" disabled={mutation.isPending}
              ...
              {mutation.isPending ? t('Guardando...') : t('Registrar pago')}
            </button>
          </div>
        </form>
      </div>
