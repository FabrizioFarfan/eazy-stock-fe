...
function CodeImage({ product, tab }) {
  ...
  if (error)   return <p className="text-sm text-red-500">{t('No se pudo generar el código.')}</p>

  return (
    <div className="flex flex-col items-center gap-3">
      <img
        src={url}
        alt={`${t(tab.label)} ${product.sku}`}
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-sm rounded-2xl bg-white shadow-xl">
        ...
        <div className="flex items-center justify-between border-b border-gray-200 px-5 py-4">
          <div>
            <h3 className="text-base font-semibold text-gray-900">{t('Códigos del producto')}</h3>
        ...
        <div className="flex flex-col items-center gap-4 px-6 py-6">
          ...
          {activeTab === 'barcode' && (
            <p className="text-center text-xs text-gray-400">
              {t('Formato Code 128 · Código: {sku}', { sku: product.sku })}
            </p>
          )}
        </div>
