...
// ═══════════════════════════════════════════════════════════════════════════
//  Mockups de UI en puro CSS/JSX. Son "capturas": se quedan en español.
// ═══════════════════════════════════════════════════════════════════════════
...
const PRODUCT_ROWS = [
  { sku: 'F2963', name: 'Cemento Sol',             unit: 'bolsa',   stock: 120, exp: null,           price: 'S/ 27.50' },
  { sku: 'F0412', name: 'Cable THW 14 AWG',        unit: 'metro',   stock: 380, exp: null,           price: 'S/ 1.90' },
  { sku: 'M0098', name: 'Leche Gloria 400g',       unit: 'unidad',  stock: 46,  exp: { d: 12 },       price: 'S/ 4.20' },
  { sku: 'F0087', name: 'Pegamento PVC 1/4 gal',   unit: 'unidad',  stock: 3,   exp: null,  low: true, price: 'S/ 12.50' },
  { sku: 'M0121', name: 'Yogurt frutado 1L',       unit: 'unidad',  stock: 18,  exp: { d: 3 },        price: 'S/ 6.50' },
  { sku: 'F2201', name: 'Tornillo 1/4" × 2"',      unit: 'paquete', stock: 524, exp: null,           price: 'S/ 0.0357' },
]
          ...
          <div className="min-w-0 flex-1 p-3 sm:p-4">
            ...
            <div className="mb-3 flex items-center gap-2">
              <div className="flex h-7 flex-1 items-center gap-1.5 rounded-lg border border-white/10 bg-white/[0.04] px-2 text-[10px] text-slate-500">
                <Search size={11} /> Buscar por nombre, código, barras…
              </div>
              <div className="flex h-7 items-center gap-1 rounded-lg border border-white/10 bg-white/[0.04] px-2 text-[10px] text-slate-300">
                <Filter size={10} /> Vence <span className="rounded bg-amber-500/20 px-1 text-[9px] text-amber-300">30 días</span>
              </div>
              <div className="hidden h-7 items-center rounded-lg bg-blue-600 px-2.5 text-[10px] font-bold text-white sm:flex">+ Nuevo</div>
            </div>
            ...
            <div className="grid grid-cols-12 gap-1 rounded-t-lg bg-white/[0.05] px-2.5 py-1.5 text-[9px] font-semibold uppercase tracking-wide text-slate-500">
              ...
              <span className="col-span-2 hidden sm:block">Unidad</span>
              <span className="col-span-2 sm:col-span-1">Stock</span>
              <span className="col-span-4 text-right sm:col-span-3">Vence · Precio</span>
            </div>
            ...
            {PRODUCT_ROWS.map((row, i) => (
              <div key={row.sku} className="lp-row grid grid-cols-12 items-center gap-1 border-b border-white/5 px-2.5 py-2" style={{ animationDelay: `${200 + i * 110}ms` }}>
                ...
                <span className="col-span-2 hidden sm:block">
                  <span className="rounded bg-white/5 px-1.5 py-0.5 text-[9px] text-slate-300">{row.unit}</span>
                </span>
                ...
                <span className="col-span-4 flex items-center justify-end gap-1.5 text-right sm:col-span-3">
                  {row.exp && (
                    <span className={`inline-flex items-center gap-0.5 whitespace-nowrap rounded-full px-1.5 py-0.5 text-[8px] font-bold ${
                      ...
                      <CalendarClock size={8} /> vence en {row.exp.d} días
                    </span>
                  )}
            ...
            <div className="mt-3 flex flex-wrap items-center gap-1.5">
              <span className="rounded-full bg-amber-500/10 px-2 py-0.5 text-[9px] font-semibold text-amber-300">2 por vencer</span>
              <span className="rounded-full bg-red-500/10 px-2 py-0.5 text-[9px] font-semibold text-red-300">1 stock bajo</span>
              <span className="rounded-full bg-blue-500/10 px-2 py-0.5 text-[9px] font-semibold text-blue-300">Código sugerido: F2964</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="lp-floaty absolute -bottom-5 -right-3 hidden max-w-[230px] rounded-xl border border-white/10 bg-[#0f172a] p-3 shadow-2xl shadow-black/50 lg:block">
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[9px] text-slate-400">"Pegamento PVC 1/4 gal" — quedan 3, mínimo 10</p>
          </div>
        </div>
      </div>
      <div className="lp-floaty-slow absolute -left-6 -top-5 hidden max-w-[220px] rounded-xl border border-emerald-400/20 bg-[#0f172a] p-3 shadow-2xl shadow-black/50 lg:block">
        <div className="flex items-start gap-2">
          ...
          <div>
            ...
            <p className="mt-0.5 text-[9px] text-slate-400">S/ 45.50 · Yape — stock actualizado</p>
          </div>
        </div>
      </div>
    </div>
...
const CASH_ROWS = [
  ...
  { label: 'Yape',          value: 820.00,  pct: 69,  icon: Smartphone, color: 'bg-violet-500' },
  { label: 'Plin',          value: 230.00,  pct: 20,  icon: Smartphone, color: 'bg-sky-500' },
      ...
      <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl">
        ...
        <div className="p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-gray-400">Cierre de caja</p>
              <p className="text-lg font-extrabold text-gray-900">Hoy · viernes 28 ago</p>
            </div>
            <span className="rounded-full bg-emerald-50 px-2.5 py-1 text-[10px] font-bold text-emerald-700 ring-1 ring-emerald-100">18 ventas</span>
          </div>

          <div className="mt-4 grid grid-cols-3 gap-2">
            {[
              { l: 'Total cobrado', v: 'S/ 2,580.50', c: 'text-gray-900' },
              { l: 'Al fiado',      v: 'S/ 250.00',   c: 'text-amber-600' },
              { l: 'Devoluciones',  v: 'S/ 0.00',     c: 'text-gray-500' },
            ].map((s) => (
          ...
          <p className="mb-2 mt-5 text-[10px] font-bold uppercase tracking-widest text-gray-400">Por medio de pago</p>
          <div className="space-y-2.5">
            {CASH_ROWS.map((r, i) => (
              <div key={r.label} className="lp-row" style={{ animationDelay: `${i * 90}ms` }}>
                <div className="mb-1 flex items-center justify-between text-xs">
                  <span className="flex items-center gap-1.5 font-semibold text-gray-700"><r.icon size={12} className="text-gray-400" />{r.label}</span>
                  <span className="font-mono font-bold text-gray-900">S/ {r.value.toLocaleString('es-PE', { minimumFractionDigits: 2 })}</span>
                </div>
          ...
          <div className="mt-5 flex items-center justify-between rounded-xl bg-[#0a0e1a] px-4 py-3 text-white">
            <span className="text-xs text-slate-300">Lo que debe haber en caja (efectivo)</span>
            <span className="font-mono text-base font-extrabold">S/ 1,180.00</span>
          </div>
        </div>
      </div>
    </div>
...
export function SupplierOrderMockup() {
  return (
    <div className="relative">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl">
        <WindowChrome url="eazy-stock.com/reports · Stock bajo" dark={false} />
        <div className="p-5">
          <div className="flex items-start justify-between gap-3">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-blue-600">Pedido al proveedor</p>
              <p className="text-lg font-extrabold text-gray-900">Distribuidora El Norte S.A.C.</p>
              <p className="text-[11px] text-gray-400">4 productos bajo mínimo · previsual editable</p>
            </div>
          ...
          <div className="mt-4 overflow-hidden rounded-xl border border-gray-200 bg-white">
            <div className="grid grid-cols-12 gap-1 bg-gray-50 px-3 py-1.5 text-[9px] font-bold uppercase tracking-wide text-gray-400">
              ...
              <span className="col-span-2 text-right">Stock/Mín</span>
              <span className="col-span-3 text-right">Pedir</span>
            </div>
            {ORDER_ROWS.map((r, i) => (
            ...
            <div className="border-t border-dashed border-gray-200 px-3 py-2 text-[10px] italic text-gray-400">
              Nota: entregar antes del sábado, por favor.
            </div>
          </div>
      ...
      <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl">
        ...
        <div className="grid gap-0 sm:grid-cols-5">
          ...
          <div className="relative sm:col-span-2 bg-[#0a0e1a] p-4">
            <p className="mb-2 flex items-center gap-1.5 text-[10px] font-bold uppercase tracking-widest text-slate-400"><ScanLine size={11} /> Escanear</p>
            ...
            <p className="mt-2 text-center text-[9px] text-emerald-400">Cámara del celular · sin lector</p>
          </div>
          ...
          <div className="sm:col-span-3 p-4">
            <div className="space-y-2">
              {[
                { n: 'Leche Gloria 400g',     q: '3 × S/ 4.20',  t: 'S/ 12.60' },
                { n: 'Cable THW 14 AWG',      q: '15 m × S/ 1.90', t: 'S/ 28.50' },
                { n: 'Tornillo 1/4" × 2"',    q: '1 paquete',   t: 'S/ 8.90' },
              ].map((it, i) => (
            ...
            <div className="mt-3 flex items-center justify-between text-[11px]">
              <span className="inline-flex items-center gap-1 rounded-md bg-rose-50 px-2 py-0.5 font-semibold text-rose-600"><Percent size={10} /> Descuento 10%</span>
              <span className="text-gray-400">Subtotal S/ 50.00</span>
            </div>

            <p className="mb-1.5 mt-3 text-[9px] font-bold uppercase tracking-widest text-gray-400">Medio de pago</p>
            <div className="flex flex-wrap gap-1.5">
              {['Efectivo', 'Yape', 'Plin', 'Tarjeta', 'Fiado'].map((m, i) => (
            ...
            <div className="lp-pop mt-3 flex items-center justify-between rounded-xl bg-emerald-600 px-4 py-2.5 text-white shadow-lg shadow-emerald-200" style={{ animationDelay: '500ms' }}>
              <span className="flex items-center gap-1.5 text-xs font-bold"><Receipt size={13} /> Cobrar</span>
              <span className="font-mono text-base font-extrabold">S/ 45.00</span>
            </div>
          </div>
        </div>
      </div>
      ...
      <div className="relative overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl">
        ...
        <div className="bg-gradient-to-br from-violet-50 to-blue-50 p-5">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-[10px] font-bold uppercase tracking-widest text-violet-600">Cliente</p>
              <p className="mt-0.5 text-lg font-extrabold text-gray-900">Rosa Huamán</p>
              <p className="text-[11px] text-gray-500">DNI 4•••••12 · +51 9•• ••• 321</p>
            </div>
            <span className="rounded-full bg-amber-100 px-2.5 py-1 text-[10px] font-bold uppercase tracking-wider text-amber-700">Debe</span>
          </div>
          <div className="mt-4 grid grid-cols-3 gap-2">
            {[
              { l: 'Deuda',      v: 'S/ 580',   c: 'text-amber-600' },
              { l: 'Límite',     v: 'S/ 1,000', c: 'text-gray-900' },
              { l: 'Disponible', v: 'S/ 420',   c: 'text-emerald-600' },
            ].map((s) => (
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <span className="inline-flex items-center gap-1.5 rounded-xl bg-[#25D366] px-3 py-1.5 text-[11px] font-bold text-white shadow-sm"><MessageCircle size={12} /> Recordar por WhatsApp</span>
            <span className="inline-flex items-center gap-1.5 rounded-xl border border-gray-200 bg-white px-3 py-1.5 text-[11px] font-bold text-gray-700"><FileText size={12} /> Estado de cuenta PDF</span>
          </div>
        </div>

        <div className="divide-y divide-gray-100">
          {[
            { type: 'SALE',    label: 'Venta al fiado',  amount: '+S/ 250', date: 'Hoy 14:30',  color: 'text-amber-600' },
            { type: 'PAYMENT', label: 'Pago · Yape',     amount: '−S/ 200', date: 'Ayer',       color: 'text-emerald-600' },
            { type: 'SALE',    label: 'Venta al fiado',  amount: '+S/ 530', date: '19 ago',     color: 'text-amber-600' },
          ].map((tr, i) => (
            <div key={i} className="lp-row flex items-center justify-between px-5 py-2.5" style={{ animationDelay: `${i * 110}ms` }}>
              <div className="flex items-center gap-3">
                ...
                <div>
                  <p className="text-xs font-semibold text-gray-900">{tr.label}</p>
                  <p className="text-[10px] text-gray-400">{tr.date}</p>
                </div>
              </div>
        ...
        <div className="border-t border-gray-100 bg-[#efeae2] p-3">
          <div className="lp-pop ml-auto max-w-[85%] rounded-lg rounded-tr-none bg-[#d9fdd3] px-3 py-2 text-[10px] leading-snug text-gray-800 shadow-sm" style={{ animationDelay: '450ms' }}>
            Hola Rosa 👋 te escribimos de <b>Ferretería San Martín</b>. Tu cuenta tiene un saldo pendiente de <b>S/ 580.00</b>. ¿Coordinamos el pago? ¡Gracias!
        ...
        <div className="flex items-center gap-2 border-b border-gray-100 bg-gray-50/60 px-5 py-3">
          {[
            ...
            <div key={s.num} className="flex items-center gap-1.5">
              ...
              <span className={`text-[11px] font-medium ${s.active ? 'text-gray-900' : s.done ? 'text-emerald-700' : 'text-gray-400'}`}>{s.label}</span>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-px">
          {[
            ...
            <div key={r.n} className={`lp-row grid grid-cols-12 gap-2 px-5 py-2 text-[11px] ${r.ok ? 'bg-emerald-50' : 'bg-amber-50'}`} style={{ animationDelay: `${i * 90}ms` }}>
              ...
              <span className={`col-span-4 truncate text-right ${r.ok ? 'text-emerald-700' : 'text-amber-700'}`}>{r.issue}</span>
            </div>
          ))}
        </div>
        <div className="flex items-center justify-between border-t border-gray-100 px-5 py-2.5 text-[10px] text-gray-500">
          <span>Historial: 4 importaciones · última hace 2 días</span>
          <span className="rounded-md bg-gray-100 px-2 py-0.5 font-semibold text-gray-700">Descargar reporte</span>
        </div>
      </div>
    </div>
  )
